...
//list of function prototype #define MAX 50
char qu[MAX][30],item[30]; int rear=0,front=0;
char catagories[][15]={"Computer","Electronics","Electrical","Civil","Mechnnical","Architecture"}; void returnfunc(void);
void addit();
void mainmenu(void); void addbooks(void); void deletebooks(void); void editbooks(void); void searchbooks(void); void issuebooks(void); void viewbooks(void); void viewbooks1(void); void returnfunc1(void);
void closeapplication(void); int getdata();
int checkid(int); int t(void);
void Password(); void issuerecord(); void loaderanim(); void suggestbooks(); voiddeq();
voidenq();
void mainmenu1(); voidoption();
void Check(); voiddisplay();
void stumenu(void); void sort(float,char); void schoice(void); void displaybooks(int);
...
//list of global files that can be accessed form anywhere in program FILE *fp,*ft,*fs;


//list of global variable int s;
char findbook;
...
{
int id;
char stname[20]; char name[20]; char Author[20]; int quantity; floatPrice;
...
gotoxy(15,9);
//textcolor(BLINK); printf("Password match"); gotoxy(17,10);
printf("Press any keytocountinue.	");
getch(); mainmenu();
}
else
{
...
gotoxy(20,3);
printf("\xB2\xB2\xB2\xB2\xB2\xB2 LIBRARY MANAGEMENT SYSTEM \xB2\xB2\xB2\xB2\xB2\xB2"); gotoxy(20,5);
printf("\xB2\xB2\xB2\xB2\xB2\xB2 YOU HAVE SUCCESFULLY LOGGED IN AS A STUDENT
\xB2\xB2\xB2\xB2\xB2\xB2");gotoxy(20,6);
...
system("cls");
gotoxy(20,3);
printf("\xB2\xB2\xB2\xB2\xB2\xB2 ENTER THE BOOK NAME AND IT'S RATING ON SCALE OF 1-10
...
gotoxy(20,3);
printf("\xB2\xB2\xB2\xB2\xB2\xB2 ENTER THE BOOK NAME AND IT'S RATING ON SCALE OF 1-10
\xB2\xB2\xB2\xB2\xB2\xB2\n"); gotoxy(25,5); scanf("%s",&n3[i3]); gotoxy(25,6);
...
gotoxy(20,3);
printf("\xB2\xB2\xB2\xB2\xB2\xB2 ENTER THE BOOK NAME AND IT'S RATING ON SCALE OF 1-10
\xB2\xB2\xB2\xB2\xB2\xB2\n"); gotoxy(25,5); scanf("%s",&n4[i4]); gotoxy(25,6);
...
gotoxy(20,3);
printf("\xB2\xB2\xB2\xB2\xB2\xB2 ENTER THE BOOK NAME AND IT'S RATING ON SCALE OF 1-10
\xB2\xB2\xB2\xB2\xB2\xB2\n"); gotoxy(25,5); scanf("%s",&n5[i5]); gotoxy(25,6);
...
gotoxy(20,3);
printf("\xB2\xB2\xB2\xB2\xB2\xB2 ENTER THE BOOK NAME AND IT'S RATING ON SCALE OF 1-10
\xB2\xB2\xB2\xB2\xB2\xB2\n"); gotoxy(25,5); scanf("%s",&n6[i6]); gotoxy(25,6);
...
void sort(float x[5],char st[5][100]){ int i,j;
float t;
char s[100]; for(i=0;i<4;i++){
for(j=0;j<4-i;j++){
if(x[j]>x[j+1])
{
...
printf("\xB2\xB2\xB2\xB2\xB2\xB2\xB2\xB2\xB2\xB2\xB2\xB2\xB2\xB2\xB2\\xB2\xB2\xB2");printf("\xB2\xB2\xB2\xB2\xB2\xB2\xB2\xB2\xB2\xB2\xB2\xB2\xB2\xB2\xB2\xB2\xB2\xB2\xB2");gotoxy(20,22);
printf("Enter your choice:"); scanf("%d",&s);
if(s==7)


mainmenu() ; system("cls");
fp=fopen("Bibek.dat","ab+"); if(getdata()==1)
{
        a.c at=catagories[s-1]; fseek(fp,0,SEEK_END);fwrite(&a,sizeof(a),1,fp);fclose(fp); gotoxy(21,14);
...
system("cls");
gotoxy(10,5);
printf("Enter the Book ID to delete:"); scanf("%d",&d); fp=fopen("Bibek.dat","rb+"); rewind(fp); while(fread(&a,sizeof(a),1,fp)==1)
{
if(a.id==d)
{
...
fp=fopen("Bibek.dat","rb+"); //open file for reading propose rewind(fp); //move pointer at the begining of file switch(getch())
{
case '1':
{
system("cls");
...
printf("Enter the bookid:");scanf("%d",&d); gotoxy(20,7); printf("Searching	");
while(fread(&a,sizeof(a),1,fp)==1)
{
if(a.id==d)
{
...
gotoxy(25,4);
printf("****Search Books ByName****"); gotoxy(20,5);
printf("Enter BookName:");scanf("%s",s);
...
system("cls");
printf("********************************ISSUESECTION**************************"); gotoxy(10,5);
...
printf("Enter the Book Id:"); scanf("%d",&t); fp=fopen("Bibek.dat","rb");
...
printf("The BOOK of ID %d is issued",a.id); a.duedate.dd=a.issued.dd+RETURNTIME; //for return date a.duedate.mm=a.issued.mm;
        a.d uedate.yy=a.issued.yy;if(a.duedate.dd>30)
{
...
gotoxy(10,15);
printf("Issue any more(Y/N):"); fflush(stdin); another=getche();
fclose(fp);
...
gotoxy(2,2);
printf("STUDENTNAME	CATEGORY ID BOOK NAME ISSUED DATE RETURNDATE");
...
if(a.id==p)
{
issuerecord(); gotoxy(10,12); printf("Pressanykey	");
getch(); issuerecord(); c=1;
}


}
fflush(stdin); fclose(fs); if(c==0)
{
gotoxy(10,8);
printf("No Record Found");
}
...
{
gotoxy(10,5);
printf("Enter book id to remove:"); scanf("%d",&b); fs=fopen("Issue.dat","rb+"); while(fread(&a,sizeof(a),1,fs)==1)
{
if(a.id==b)
{
...
{
gotoxy(10,12);
printf("Do You Want to Removeit?(Y/N)"); if(getch()=='y')
{
...
gotoxy(1,1);
printf("*********************************BookList*****************************"); gotoxy(2,2);
printf("CATEGORY	IDBOOKNAME	AUTHOR	QTY	PRICE	RackNo");j=4;
...
gotoxy(15,16);
printf("Modify another Record?(Y/N)"); fflush(stdin);
another=getch() ;
...
a:
if(getch()==13) //allow only use of enter mainmenu();
else goto a;
}
...
a:
if(getch()==13) //allow only use of enter mainmenu1();
else goto a;
}
int getdata()
{
int t;gotoxy(20,3);
printf("Enter the Information Below"); gotoxy(20,4);
printf("\xB2\xB2\xB2\xB2\xB2\xB2\xB2\xB2\xB2\xB2\xB2\xB2\xB2\xB2\xB2"); printf("\xB2\xB2\xB2\xB2\xB2\xB2\xB2\xB2\xB2\xB2\xB2\xB2");
gotoxy(20,5); printf("\xB2");gotoxy(46,5);printf("\xB2"); gotoxy(20,6); printf("\xB2");gotoxy(46,6);printf("\xB2"); gotoxy(20,7); printf("\xB2");gotoxy(46,7);printf("\xB2"); gotoxy(20,8); printf("\xB2");gotoxy(46,8);printf("\xB2"); gotoxy(20,9); printf("\xB2");gotoxy(46,9);printf("\xB2"); gotoxy(20,10); printf("\xB2");gotoxy(46,10);printf("\xB2"); gotoxy(20,11); printf("\xB2");gotoxy(46,11);printf("\xB2"); gotoxy(20,12);
printf("\xB2\xB2\xB2\xB2\xB2\xB2\xB2\xB2\xB2\xB2\xB2\xB2\xB2"); printf("\xB2\xB2\xB2\xB2\xB2\xB2\xB2\xB2\xB2\xB2\xB2\xB2\xB2");gotoxy(21,5);
printf("Category:"); gotoxy(31,5);printf("%s",catagories[s-1]);gotoxy(21,6);
printf("Book ID:\t"); gotoxy(30,6); scanf("%d",&t); if(checkid(t) == 0)
{
gotoxy(21,13);
printf("\aThe book id already exists\a"); getch();
mainmenu(); return 0;
}
a.id=t; gotoxy(21,7);
